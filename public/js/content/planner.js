// Date planner — shared client copy (used when running on Supabase).
// Energy: 1 = low, 2 = normal, 3 = adventurous.
...
const LIB = [
  ...
  { kind: 'game', icon: '⚡', title: 'This or that: lightning round', game: 'thisthat', desc: 'No thinking allowed. First instinct wins.', minutes: 8, moods: ['playful', 'chill', 'low'], energy: 2 },
...
export function planDate({ minutes = 45, mood = 'chill', energy = 2, notes = '' } = {}) {
  ...
  const eligible = LIB.filter(a => (a.moods.includes('*') || a.moods.includes(mood)) && a.energy <= energy + 1 && !a.closer);
  ...
  budget -= 3 + gratitude.minutes;

  const chosen = [];
  for (const a of pool) {
    if (budget < a.minutes) continue;
    if (chosen.some(c => c.kind === a.kind)) continue;
  ...
  if (energy === 1) chosen.sort((a, b) => a.energy - b.energy);
  const all = [...chosen, gratitude];
  if (mood === 'low' && !all.some(a => a.kind === 'rain')) {
    all.splice(all.length - 1, 0, { kind: 'rain', icon: '🌧️', title: 'Sit together during the rain', desc: 'Just Stay, rain on. Words optional.', minutes: 8, energy: 1 });
  }
